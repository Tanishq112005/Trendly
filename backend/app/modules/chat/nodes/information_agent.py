...
prompt = ChatPromptTemplate.from_messages([
    ("system", "You are an extraction assistant. Extract the user's email and phone number from their latest message."),
...
def verify_auth_node(state: AgentState) -> dict:
    ...
    if not email and not phone:
        updates["messages"] = [AIMessage(content="To assist you better, I need to know your email and phone number first. Can you please provide me with both?")]
        return updates
    if not email:
        updates["messages"] = [AIMessage(content="I have your phone number, but I still need your email address to proceed. Can you please provide it?")]
        return updates
    if not phone:
        updates["messages"] = [AIMessage(content="I have your email, but I still need your phone number to proceed. Can you please provide it?")]
        return updates
...
def route_after_extract(state: AgentState) -> list[str]:
    ...
    if state.get("temp_extracted_email") and state.get("temp_extracted_email") != "UNKNOWN":
        destinations.append("save_email")
        
    if state.get("temp_extracted_phone") and state.get("temp_extracted_phone") != "UNKNOWN":
