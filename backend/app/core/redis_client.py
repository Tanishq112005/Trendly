...
class RedisManager:
    ...
    async def publish_ticket(cls, session_id: str, ticket_id: int):
        ...
        try:
            ...
            logging.info(f"Published ticket event for session {session_id}, ticket {ticket_id}")
        except Exception as e:
            logging.error(f"Failed to publish ticket event to Redis (network drop or timeout): {e}")
